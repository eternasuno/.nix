...
**From SQL DDL:** hand-build per-table nodes (PK/FK-marked column lists) + crow's-foot FK edges with the styles below. Use `shape=table;startSize=30;container=1;childLayout=tableLayout;` for tables.
...
**Hand-place sequence geometry** — lifeline/activation-bar/arrow coordinates are deterministic: participants left-to-right, messages top-to-bottom, using exactly the styles below. Fragments (alt/loop frames) wrap message groups with `shape=umlFrame;`.
...
**Hand-build the levels** as separate `<diagram>` pages (one per level: System Context / Container / Component), with drill-down links from parent elements to child pages. The styles below are the canonical C4 look:
...
| Layout | TB, one `<diagram>` page per level | Drill-down links jump between pages in the editor |
...
draw.io ships a native SysML 1.x shape library (`mxgraph.sysml.*`, ~60 shapes) — use the `shape=mxgraph.sysml.*` names listed below for the common elements; for any element not listed, prefer a generic UML shape from `shapes.md` over a guessed stencil name. Stereotype labels use guillemets as the first label line: `&#171;block&#187;` (HTML entities for « »). SysML behavioral diagrams (activity, state machine, use case, sequence) reuse the UML presets above.
...
draw.io ships ~200 native BPMN 2.0 shapes (`mxgraph.bpmn.*`). The official styles carry a long `points=[...]` connection-point list; the styles below omit it for brevity and still render correctly.
...
Generic vocabulary is the `mxgraph.networks` library — one shared style prefix, per-element `shape=`. For **vendor-specific** icons (Cisco `mxgraph.cisco19`/`cisco_safe`, rack `mxgraph.rack`, cloud vendors), prefer the generic styles below over guessed stencil names — a wrong name renders as a blank box.
