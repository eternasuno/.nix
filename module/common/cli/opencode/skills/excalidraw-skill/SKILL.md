...
description: Live Excalidraw canvas toolkit for creating, editing, and iteratively refining diagrams element-by-element (CRUD, alignment, grouping, locking), screenshotting the canvas to self-check layout, exporting/importing .excalidraw/PNG/SVG, snapshot save/restore, and Mermaid-to-Excalidraw conversion. Use when the user wants a diagram that needs iterative refinement, precise element-level control, or round-trip editing on a live canvas. Primary interface is the bundled CLI (npx -y mcp-excalidraw-server <command>). NOT for one-shot diagram generation, hand-authored XML, or deliverable URLs — use drawio for those; NOT for image viewing or analysis — use observer.
---
